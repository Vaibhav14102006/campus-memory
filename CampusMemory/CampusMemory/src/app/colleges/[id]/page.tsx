...
export default function CollegePage({ params }: CollegePageProps) {
  ...
  const resolvedProblems = savedProblems ? JSON.parse(savedProblems).filter((p: any) => p.status === "resolved").length : 0;
  const activeProblemsList = savedProblems ? JSON.parse(savedProblems).filter((p: any) => p.status !== "resolved" && p.status !== "prevented") : [];
  const highRiskAlerts = savedAlerts ? JSON.parse(savedAlerts).filter((a: any) => a.riskLevel === "red").length : 0;
